...
export const cartSlice = createSlice({
	...
	reducers: {
		...
		deleteItem: (state, { payload }) => {
			/* This is creating a copy of the current state of the cart. */
			const items = createCopy(current(state).cartItems).filter(
				(el) => el.id !== payload
			);
			state.cartItems = items;
			state.cartItemsCount = sumByKey(items, 'qty');
		...
		incrementItem: (state, { payload }) => {
			/* This is creating a copy of the current state of the cart. */
			const items = createCopy(current(state).cartItems).map((el) =>
				el.id === payload
					? { ...el, qty: el.qty + 1, amount: el.amount + el.price }
					: el
			);
		...
		decrementItem: (state, { payload }) => {
			...
			const items = createCopy(current(state).cartItems).map((el) =>
				el.id === payload
					? {
							...el,
							qty: el.qty - 1 !== 0 ? el.qty - 1 : 1,
							amount:
								el.amount - el.price !== 0
									? el.amount - el.price
									: el.price,
					  }
					: el
...
export const { addItem, removeItem, deleteItem, incrementItem, decrementItem } =
	cartSlice.actions;
